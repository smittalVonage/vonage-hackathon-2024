...
const SignInOtp = () => {
  ...
  const handleVerifyOtp = async () => {
    const { whatsappNumber } = location.state;
    ...
    try {
      ...
      if (response.status === 200) {
        navigate('/dashboard', { state: { whatsappNumber } });
      } else {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col justify-center items-center">
      ...
      {error && (
        <div className="fixed top-4 right-4 bg-red-500 text-white px-4 py-2 rounded shadow-lg transition-opacity duration-500 ease-in-out opacity-100">
          {error}
        </div>
      )}
    </div>
